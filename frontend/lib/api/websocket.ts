...
class WebSocketClient {
  ...
  constructor() {
    // Server-side fallback (SSR) - use a placeholder
    ...
    // Priority 2: Browser environment - auto-detect based on current page
    const protocol = window.location.protocol === "https:" ? "wss" : "ws";
    const hostname = window.location.hostname;
    
    // In development (localhost), connect to separate backend port
    // In production (deployed), connect to same host (assuming backend on same domain)
