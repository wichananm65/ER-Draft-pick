...
export default function RecentActionBanner({ lastAction, heroes }: Props) {
  ...
  const borderColorClass = lastAction.side === 'left' ? 'border-blue-500' : 'border-red-500';
  const verb = lastAction.type === 'pick' ? t('pick') : t('ban');
  const heroImage = hero ? `/characters/${hero.name}.webp` : '';
  const isBan = lastAction.type === 'ban';
  ...
  return (
    <div className="shrink-0 mt-0 mb-4 w-full flex justify-center">
      <div className="w-full max-w-[640px] px-6 py-3 rounded-2xl flex items-center gap-4 bg-linear-to-br from-gray-800/60 to-gray-900/60 border border-gray-700 shadow-lg backdrop-blur-sm">
        ...
        {hero && (
          <div className={`w-12 h-12 rounded-lg overflow-hidden border-2 ${borderColorClass} shrink-0`}>
            <img 
              src={heroImage} 
              alt={hero.name}
              className={`w-full h-full object-cover ${isBan ? 'grayscale opacity-60' : ''}`}
              onError={(e) => {
