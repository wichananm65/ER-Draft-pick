...
class Room {
  ...
  updateState(newState) {
    if (newState.currentPhase !== undefined)
      this.currentPhase = newState.currentPhase;
    if (newState.actionCount !== undefined)
      this.actionCount = newState.actionCount;
    ...
    if (Object.prototype.hasOwnProperty.call(newState, 'hasBeenSaved'))
      this.hasBeenSaved = !!newState.hasBeenSaved;
  }
}
        ...
        if (side === "left") {
          ...
          if (ws._isOwner) {
            ...
            participants.forEach((p) => {
              try {
                p.send(JSON.stringify({ type: "room-closed", roomCode }));
              } catch {}
              try {
                p.close();
              } catch {}
...
function handleReadyToRestart(ws, message) {
  ...
  if (readySet.has('left') && readySet.has('right')) {
    ...
    // Toggle swapSides so clients flip positions/colors (teams swap visual sides)
...
// Start server
server.listen(PORT, () => {
  console.log(`\n🚀 Pick/Ban Server running on ${NODE_ENV} mode`);
